...
- **Reaching the Windows PC over Tailscale (e.g. container on Unraid)** — use the PC's Tailscale IP (`100.x.y.z`, from `tailscale ip` on that PC) in `ZRO_BRIDGE_URL` / `DOGEGEN_AGENT_URL`, not its `.ts.net` MagicDNS name. A container on a plain bridge network doesn't have access to Tailscale's DNS resolver, so the MagicDNS hostname resolves fine from your laptop but fails to resolve from inside the container. If you'd rather keep using the MagicDNS name, add `--dns=100.100.100.100` to the container's run args (Unraid: Extra Parameters) so it can reach Tailscale's resolver.
