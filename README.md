...
1. On the Windows PC, get both companion services running. Either:
   - **Download and run** — grab `windows-companion-tools.zip` from the [Releases page](https://github.com/jweber93/tv-calibration/releases), unzip, copy `bridge.example.json` → `bridge.json` and `agent.example.json` → `agent.json`, then double-click `zro-bridge.exe` and `dogegen-agent.exe`. No Python install required.
   - **Run from source** — `tools/zro-bridge/start.bat` (copy `bridge.example.json` → `bridge.json` first; set `"backend": "argyll"` if you're using ArgyllCMS instead of ColourSpace) and `tools/dogegen-agent/start.bat` (copy `agent.example.json` → `agent.json` first). Requires Python 3.10+.
...
- **Manual apply** (works on every TV): the loop tells you exactly which CMS control to change and by how much; you make the change on the TV, click **"I made the change — Remeasure"**, and the loop re-measures.
- **Auto apply** (ADB-capable TVs): the loop pushes the correction directly via ADB with read-back verification. If an ADB write or read-back fails for a given step, that one step falls back to a manual instruction (same confirm-and-remeasure flow) instead of aborting the run.
...
- Whenever a step needs the user to act (manual mode, or an ADB fallback), the loop pauses — re-measuring before the change is actually made would just report a stale reading — and resumes as soon as the step is confirmed.

The Color Tuner step's **Autocal** card runs this end-to-end in the UI: pick apply mode and iteration cap, hit **Start Autocal**, and watch each primary's live ΔE and status (`Measuring… / Waiting on you / Converged / Keep going`). **Stop** cancels cooperatively — a paused run also unblocks on Stop.
...
POST /api/session/{sid}/autocal/confirm  → unblocks a run paused on a manual (or ADB-fallback)
                                            step once the user has made the change on the TV
GET  /api/session/{sid}/autocal/history  → full per-primary iteration history for the most recently
                                            completed run (error, ΔE, damping, gain source, oscillation/
                                            stall flags, whether the step required user action) —
                                            useful for tuning damping or diagnosing why a loop
                                            converged, stalled, or oscillated
GET  /api/session/{sid}/autocal/stream   → SSE: autocal_start, autocal_iteration, autocal_waiting,
                                            autocal_colour_done, autocal_done, autocal_error
```

`autocal_waiting` fires (with `{colour, iteration}`) whenever the loop is paused on `/autocal/confirm`; `autocal_iteration` payloads include `delta_e` (the ΔE from that iteration's measurement) and `requires_user_action` (whether that step's apply was manual/fell back to manual).
