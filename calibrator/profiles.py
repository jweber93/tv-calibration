...
def _build_u8g_profile() -> TVProfile:
    ...
    return TVProfile(
        ...
        CONFIGURE_FOR_CAL=[
            ("Local Dimming", "High for HDR calibration/validation; use Off only as a troubleshooting cross-check if measurements look unstable"),
        ...
        PICTURE_CONTROL_GUIDE=[
            ("Brightness (Black Level)", "Show a 5% gray patch. Raise Brightness until 5% is just visible, then lower it one click if blacks look lifted."),
            ("Contrast (White Level)", "Show a 95% gray patch. Raise Contrast until near-white detail starts to clip, then back it down 1-2 clicks."),
            ("Backlight", "Show a 100% white patch and use Backlight to hit the luminance target for the current mode."),
        ],
        ...
        RESET_BEFORE_CAL=[
            ("White Balance — Red/Green/Blue Gain",   "0  (neutral; eliminates any prior calibration offsets in highlights)"),
            ("White Balance — Red/Green/Blue Offset", "0  (neutral; eliminates any prior calibration offsets in shadows)"),
            ("Colour Tuner — Hue (all colours)",      "0  (no colour rotation)"),
            ("Colour Tuner — Saturation (all colours)", "0  (no saturation push/pull)"),
            ("Colour Tuner — Brightness (all colours)", "0  (no luminance offset per colour)"),
            ("Brightness (black level)",               "50 (factory default; wrong value clips blacks or lifts the floor)"),
...
def _build_tcl7105x_profile() -> TVProfile:
    ...
    return TVProfile(
        ...
        DISABLE_BEFORE_CAL=[
            ...
            ("Dynamic Tone Mapping", "Off for SDR; leave on ST.2084 for HDR10 measurement"),
        ],

        CONFIGURE_FOR_CAL=[
            ("Backlight", "Adjust to reach target peak luminance"),
            ("Brightness", "50 (factory default for Movie mode)"),
            ("Contrast", "90 (factory default for Movie mode; back down if 100% whites clip)"),
            ("Color Temperature", "Warm (closest to D65 on this panel)"),
            ("Color Space", "Auto for SDR Rec.709; Native or Wide for HDR gamut measurement"),
            ("Gamma", "2.2 for SDR (BT.1886 is not offered; 2.2 is the closest equivalent)"),
            ("HDMI Mode", "Enhanced Signal (Settings → TV Inputs → HDMI) for 4K/HDR sources"),
        ],

        PICTURE_CONTROL_GUIDE=[
            ("Brightness (Black Level)", "Show a 5% gray patch. Raise Brightness until 5% gray is just visible, then lower one click if blacks look lifted."),
            ("Contrast (White Level)", "Show a 95% white patch. Raise Contrast until near-white detail begins to clip, then back off 1-2 clicks."),
            ("Backlight", "Show a 100% white patch. Use Backlight to hit the target luminance (e.g. 120 nits for SDR)."),
        ],
        ...
        RESET_BEFORE_CAL=[
            ("White Balance — Red/Green/Blue Gain",    "0  (neutral on the −50 to +50 scale)"),
            ("White Balance — Red/Green/Blue Offset",  "0  (neutral on the −50 to +50 scale)"),
            ("Color Tuner — Hue (all colours)",        "0  (no colour rotation)"),
            ("Color Tuner — Saturation (all colours)", "0  (no saturation push/pull)"),
            ("Color Tuner — Brightness (all colours)", "0  (no per-colour luminance offset)"),
...
def _build_lg_oled55b7a_profile() -> TVProfile:
    ...
    return TVProfile(
        ...
        PICTURE_CONTROL_GUIDE=[
            ("Brightness (Black Level)", "Show a 5% gray patch. Raise Brightness until 5% is just barely visible, then lower one click if blacks look lifted. OLED blacks are self-emissive — even a small positive offset is visible."),
            ("Contrast (White Level)", "Show a 95% white patch. Raise Contrast until near-white detail begins to clip, then lower 1-2 clicks. On OLED, 85-90 is a typical safe upper limit."),
            ("OLED Light", "Show a 100% white patch and use OLED Light to hit the target luminance (e.g. ~120 nits SDR at OLED Light 35-45)."),
        ],
        ...
        RESET_BEFORE_CAL=[
            ("White Balance — Red/Green/Blue Gain",    "0  (LG scale is typically -50 to +50; 0 is neutral)"),
            ...
            ("Brightness (black level)",               "50 (LG ISF Expert default; prevents black crush or lift)"),
            ("Contrast (white level)",                 "85 (ISF Expert Dark Room default; avoids highlight clip on OLED)"),
            ("Sharpness",                              "0  (OLED panels are already razor-sharp; any sharpening distorts edge patches)"),
            ("Colour Filter",                          "Off (must be off; it tints the entire image for colour-channel isolation)"),
        ],
    )
...
def _build_vizio_v4k55m_profile() -> TVProfile:
    ...
    return TVProfile(
        ...
        PICTURE_CONTROL_GUIDE=[
            ("Brightness (Black Level)", "Show a 5% gray patch. Raise Brightness until 5% is just barely visible, then lower one click if blacks look elevated."),
            ("Contrast (White Level)", "Show a 95% white patch. Raise Contrast until detail starts to clip, then back off 1-2 clicks. Default of 50 is usually safe."),
            ("Backlight", "Show a 100% white patch. Use Backlight to reach the target luminance (e.g. 120 nits SDR)."),
        ],
        ...
        RESET_BEFORE_CAL=[
            ("White Balance — Red/Green/Blue Gain (service menu)",   "Default/factory value — write down originals before changing anything"),
            ("White Balance — Red/Green/Blue Offset (service menu)", "Default/factory value — write down originals before changing anything"),
            ...
            ("Color Enhancement / Color AI",                         "Off — this is separate from the disable list and easy to miss"),
        ],
        ...
        SERVICE_MENU_CONTROLS=[
            ("White Balance → Red/Green/Blue Gain", "Fine-tune highlight white balance (not exposed in normal menus on V-Series)"),
            ("White Balance → Red/Green/Blue Offset", "Fine-tune shadow white balance"),
            ("Color Tuner → per-colour Hue/Sat/Brightness", "Per-colour CMS adjustments with finer resolution than user menu"),
            ("Panel Calibration → Uniformity", "Factory uniformity correction (do not change unless you know what you are doing)"),
        ],
    )
