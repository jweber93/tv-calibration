    ...
    if pattern_generator == "dogegen":
        if mode == "SDR":
            return [
                ...
                {"step": "Configure Dogegen", "detail": "Launch Dogegen and put it on the same HDMI/signal path you will calibrate. Set it for SDR Rec.709 output and confirm the TV is not in HDR mode."},
                {"step": f"Connect {app_name} to Dogegen", "detail": "In ColourSpace, select Dogegen as the active patch generator / TPG path you will use for measurements."},
                {"step": "Configure patch settings", "detail": "Set patch size to ~30%, background to black, and confirm the app's signal-range setting matches Dogegen's output range."},
        ...
        if mode == "HDR10":
            return [
                ...
                {"step": "Configure Dogegen for HDR10", "detail": "Set Dogegen to an HDR10 / PQ / Rec.2020 output mode on the exact HDMI path you will calibrate. For PC-driven HDR, use 10 bpc and RGB 4:4:4 Full. Confirm your TV shows an HDR10 or HDR badge before measuring."},
                {"step": f"Connect {app_name} to Dogegen", "detail": "Use Dogegen as the active patch generator / TPG path in ColourSpace and keep the HDR signal path live while measuring."},
                {"step": "Configure patch settings", "detail": "Set a 10% HDR window on black background, and make sure the app's signal-range selection matches Dogegen's actual output range. For Dogegen HDR on PC this is usually Full Range. Use 3% only as a secondary peak-highlight cross-check."},
